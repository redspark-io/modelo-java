...
public class CityControllerTest extends ApplicationTest {
	...
	public void testeListWithPagination() {
		...
		Page<CityDTO> page = get("/city").queryParam("page", "0").queryParam("size", "2").status(HttpStatus.OK)
	...
	public void testCreate() throws JsonProcessingException, IOException {
		...
		ResponseEntity<CityDTO> response = post("/city").formParam("name", name).formParam("state", state)
		    .formParam("country", country).status(HttpStatus.CREATED).getResponse(CityDTO.class);
	...
	public void testUpdate() {
		...
		ResponseEntity<CityDTO> response = put("/city/%s", c1.getId()).formParam("name", name).formParam("state", state)
		    .formParam("country", country).status(HttpStatus.OK).getResponse(CityDTO.class);
	...
	public void testUpdateNotFound() {
		...
		saveall(bruno);
		signIn(bruno);
		put("/city/1").formParam("name", "name").formParam("state", "state").formParam("country", "country")
	...
	public void testeCreateInvalidParam() {
		...
		post("/city").status(HttpStatus.BAD_REQUEST).getResponse();
	}
}
